...
def main() -> None:
    ...
    parser.add_argument(
        "--mode",
        choices=["centered", "gt-contained"],
        default="centered",
        help="Crop around accident center or only ensure GT bbox is fully included",
    )
    ...
    labels_output_name = (
        "labels_crop25.csv"
        if args.mode == "centered"
        else "labels_crop25_gt_contained.csv"
    )
    ...
    metadata_output_name = (
        "test_metadata_crop25.csv"
        if args.mode == "centered"
        else "test_metadata_crop25_gt_contained.csv"
    )
