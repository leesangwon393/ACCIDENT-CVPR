...
DEFAULT_ADAPTER_PATH = "/root/Desktop/workspace/yuyeon/Experiments/16. RB-FT/outputs/experiment_artifacts/adapter_stage1_fix_version_with_mlp_e3_lang5e5_vis1e5_wd001_b1_ga8/final"
DEFAULT_VIDEO_DIR = "/root/Desktop/workspace/yuyeon/raw/accident/sim_dataset/videos"
...
def preprocess_video(video_path: str, max_frames: int = 5) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        if frame_count % step == 0:
            ...
            samples.append({"image": Image.fromarray(frame_rgb), "timestamp": frame_count})
...
def _normalize_type_filter(type_filter: Optional[str]) -> Optional[List[str]]:
    if not type_filter:
        return None
    parts = [token.strip() for token in type_filter.split(",")]
    selected = [token for token in parts if token]
    return selected or None


def select_videos_by_type(
    video_dir: str,
    videos_per_type: int,
    ...
    for type_dir in sorted(p for p in root.iterdir() if p.is_dir()):
        ...
        all_mp4s = sorted(type_dir.rglob("*.mp4"))
        chosen = all_mp4s[:videos_per_type]

        rel_paths = [str(path.relative_to(root)) for path in chosen]
        selected_by_type[type_name] = rel_paths

        for path in chosen:
            selected_pairs.append((type_name, str(path)))

    if not selected_pairs:
        if allowed_types:
            raise FileNotFoundError(
                f"No .mp4 files found for requested types {allowed_types} in: {video_dir}"
            )
        raise FileNotFoundError(f"No .mp4 files found under type directories in: {video_dir}")

    return selected_pairs, selected_by_type


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Stage1 object extraction inspection (sampled per top-level type)"
    )
    ...
    parser.add_argument("--videos-per-type", type=int, default=5)
...
def main() -> None:
    ...
    print(f"[SYSTEM] Videos per type : {args.videos_per_type}")
    print(f"[SYSTEM] Type filter     : {args.type_filter}")

    model, processor = load_model_and_processor(args)

    selected_videos, selected_by_type = select_videos_by_type(
        video_dir=args.video_dir,
        videos_per_type=args.videos_per_type,
        type_filter=args.type_filter,
    )
    print(f"[SYSTEM] Found {len(selected_videos)} video(s) to analyze.")
    ...
    for idx, (type_name, v_path) in enumerate(selected_videos, start=1):
        v_name = os.path.relpath(v_path, args.video_dir)
        print(f"\n{'=' * 70}")
        print(f"[{idx}/{len(selected_videos)}] Processing: {v_name}")
        ...
        try:
            ...
            type_output_dir = os.path.join(args.output_dir, type_name)
            os.makedirs(type_output_dir, exist_ok=True)
            save_path = os.path.join(type_output_dir, f"{Path(v_path).stem}_report.json")
